...
__feature_list__ = [
    ...
    "var_tfidf",
    "query_likelihood"  
    "first_query_term_reciprocal_position" 
]
...
class FeatureList:
    ...
    f14 = "mean_tfidf"
    f15 = "var_tfidf"
    f16 = "query_likelihood"  
    f17 = "first_query_term_reciprocal_position" 
...
class FeatureExtraction:
    ...
    def extract(self, qid: int, docid: int, **args) -> dict:
        ...
        for term in set(query) & set(doc_terms):
            ...
            p_coll = (self.documents.cf.get(term, 0) + alpha) / (self.documents.total_terms + alpha * len(self.documents.cf))
            p_term_doc = (tf + alpha) / (doclen + alpha * len(doc_terms))
            query_likelihood += np.log(p_term_doc * p_coll + 1e-10)  # Log-probability
        ...
        # new features
        non_stopword_query = [term for term in query if term not in self.queries.preprocessor.sw]

        for term in set(non_stopword_query) & set(doc_terms):
                p_coll = (self.documents.cf.get(term, 0) + alpha) / (self.documents.total_terms + alpha * len(self.documents.cf))
                tf = doc_terms.count(term)
                p_term_doc = (tf + alpha) / (doclen + alpha * len(doc_terms))
                query_likelihood += np.log(p_term_doc * p_coll + 1e-10) 

        features[FeatureList.f16] = query_likelihood

        min_position = next((idx for idx, term in enumerate(doc_terms) if term in non_stopword_query), float('inf'))
        features[FeatureList.f17] = 1.0 / (min_position + 1) if min_position != float('inf') else 0.0
